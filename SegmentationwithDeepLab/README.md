...
# Model or Checkpoint saving loaction should be:
```
/markov/markov_model or 
/current_working_dir/markov_model
```

# Dataset
```
/hdfs/bigred3ns/user/SVMLP17/hive/datasets/VOCdevkit
```

# Run Training on MARKOV BIGRED NOTEBOOK 
```
./run_training.sh
```

# Launch Training & Deploy model using markov- tmlp-sdk
[!(TMLP_SDK launchinng training )](https://git.target.com/markov-onboarding/trainer-DeepLabV3Plus-Pytorch/blob/master/programable_training_serving_using_markov_sdk.ipynb)
...
# Customize training and datasets 

### 2. Prepare Datasets

#### 2.1 Standard Pascal VOC
You can run train.py with "--download" option to download and extract pascal voc dataset. The defaut path is '/hdfs/bigred3ns/user/SVMLP17/hive/datasets':

```
/hdfs/bigred3ns/user/SVMLP17/hive/datasets
    /VOCdevkit 
        /VOC2012 
            /SegmentationClass
            /JPEGImages
...
/hdfs/bigred3ns/user/SVMLP17/hive/datasets/train_aug.txt* includes the file names of 10582 trainaug images (val images are excluded)
